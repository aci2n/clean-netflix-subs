...
const searchParams = new URLSearchParams(unsafeWindow.location.search);
...
(function() {
  const episodeIds = Array.from(document.querySelectorAll(".ptrack-content > a"))
    ...
    .filter(id => !isNaN(id));
  return window.location.origin + '/watch/' + episodeIds[0] + '?subtitle-batch&subtitle-queue=' + episodeIds.slice(1).join(',');
}())
*/
...
let subCache = {};
let titleCache = {};
const batch = searchParams.has('subtitle-batch');
const queue = searchParams.get('subtitle-queue') ? searchParams.get('subtitle-queue').split(',') : [];
console.log('Batch download queue', queue);
...
const processSubInfo = result => {
  console.log('Processing sub info', result);
...
const processMetadata = data => {
  ...
  console.log('Processing metadata for episode', {currentEpisodeId: result.currentEpisode, data, title, type});
  if(type === 'show') {
    let prev = null;
    console.log('Seasons', result.seasons.map(s => s.seq));
    for(const season of result.seasons) {
      console.log('Episodes in season', season.seq, season.episodes.map(e => e.seq));
      for(const episode of season.episodes) {
        if (result.currentEpisode === episode.id) {
          console.log('Current episode', {season: season.seq, episode: episode.seq, id: episode.id});
        }
        if (result.currentEpisode === prev) {
		      // keep track of the next episode to use with batch downloads
          console.log('Next episode', {season: season.seq, episode: episode.seq, id: episode.id});
...
const _download = async _zip => {
  ...
  for(const lang of filteredLangs) {
    ...
    while(urls.length > 0) {
      ...
      console.log('Downloading subs from url', url);
      ...
      if(data.length > 0) {
        console.log('Downloaded successfully', {url, lang, extension, data});
...
const downloadAll = async () => {
  ...
  if (nextEpisodeId) {
    ...
    const nextQueueEpisodeUrl = unsafeWindow.location.origin + '/watch/' + queue[0] + '?subtitle-batch&subtitle-queue=' + queue.slice(1).join(',');
    console.log('Redirecting to next queued episode', nextQueueEpisodeUrl);
    unsafeWindow.location.href = nextQueueEpisodeUrl;
  } else {
    ...
    console.log('Done downloading batch, redirecting to main page to avoid spoilers');
